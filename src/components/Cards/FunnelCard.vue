...
<template>
  <div  class="card bg-white rounded-3  border-0">
    <div v-if="dropDownState" style="top: 45px; right: 60px; border: 1px solid #EBEBEB" class="card-body z-3 bg-white rounded-2 position-absolute  col-10 col-md-6 d-flex">
      <div class="w-100">
        <div @click="handleModalState" :class="['py-2 px-3 drop-up-option rounded-3 w-100 d-flex justify-content-between', userColorData.text]">
                  <p class="m-0">Edit</p>
                  <i class="bi bi-pencil-fill"></i>
        </div>
        <div @click="duplicateFunnel" :class="['mt-2 drop-up-option w-100 py-2 px-3 rounded-3 d-flex justify-content-between', userColorData.text]">
    ...
    <div @click="routeToSteps" class="card-body">
      <div class="">
        <div class="d-flex mx-1 align-items-center justify-content-between ">
          <h4 :class="['fs-6 fw-bold m-0', userColorData.text]">{{ handleFunnelNameSize()  }}</h4>
          <p :class="['fs-6 fw-bold m-0', userColorData.text]">Code: {{ funnelData.id }}</p>
        </div>
        <div class="my-2 d-flex align-items-center justify-content-between ">
          <button @click.stop="handleFavoriteState" class="bg-transparent border-0"><i :class="[ funnelData.favorite ? 'bi-pin-angle-fill' : 'bi-pin-angle', 'bi py-1 px-2 rounded-2 text-white', userColorData.color ]"></i></button>
          <button class="bg-transparent border-0"><i @click.stop="handleDropDownState" :class="['bi py-1 px-2 rounded-2  button-option text-white bi-three-dots', userColorData.color]"></i></button>
        </div>
      </div>
      <div class="mx-1 pt-3 pb-4">
        <p :class="['m-0 fs-6', userColorData.text]"> {{ funnelData.description.length > 60 ? funnelData.description.slice(0, 60) + '...' : funnelData.description }} </p>

      </div>
      <div style="bottom: 15px; right: 23px" class="position-absolute">
        <p :class="['m-0', userColorData.text]">{{ funnelData.date }}</p>
      </div>
    </div>
  </div>
</template>
